...
void MoveJoint::UpdateLogic(const systems::Context<double>& context,
  ...
  if (init_new_traj) {
    ...
    const Eigen::VectorXd q1 = input_traj.value(input_traj.end_time());
    ...
    drake::log()->info("t0 {} t1 {}", input_traj.start_time(),
                       input_traj.end_time());
    drake::log()->info("q_now: {}", q.transpose());
    drake::log()->info("q_start: {}", q0.transpose());
    drake::log()->info("q_end: {}", q1.transpose());
...
MoveToolStraight::MoveToolStraight(
  ...
  const PiecewiseCartesianTrajectory<double> dummy_traj =
  ...
  trajectory_input_port_ = &this->DeclareAbstractInputPort(
      "tool_trajectory",
      Value<PiecewiseCartesianTrajectory<double>>(dummy_traj));
  command_traj_index_ = this->DeclareAbstractState(
      AbstractValue::Make<PiecewiseCartesianTrajectory<double>>(dummy_traj));
}
...
void MoveToolStraight::UpdateLogic(const systems::Context<double>& context,
                                   systems::State<double>* state) const {
  const auto& input_traj =
      get_trajectory_input().Eval<PiecewiseCartesianTrajectory<double>>(
          context);
  auto& cmd_traj =
  ...
  if (init_new_traj) {
    ...
    systems::BasicVector<double>& x_current = get_mutable_x_command(state);
    x_current.SetZero();
    x_current.get_mutable_value().head(num_q) = q;
  }
}
