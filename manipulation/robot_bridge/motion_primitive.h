...
class MoveToolStraight final : public MoveTool {
 public:
   ...
   * PiecewiseCartesianTrajectory<double>. I am not married to this, just
 ...
 private:
  const PiecewiseCartesianTrajectory<double>& get_command_traj(
      const systems::State<double>& state) const {
    return state.get_abstract_state<PiecewiseCartesianTrajectory<double>>(
        command_traj_index_);
  }
